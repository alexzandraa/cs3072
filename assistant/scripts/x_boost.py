...
features = ["Occupation", "Age_Bracket", "Gender", "Ethnicity", "Education_Level",
...
categorical_features = ["Occupation", "Age_Bracket", "Gender", "Ethnicity", "Education_Level"]
...
joblib.dump(best_model, os.path.join(model_folder, "xgboost_model.pkl"))
joblib.dump(scaler, os.path.join(model_folder, "scaler_xgb.pkl"))
joblib.dump(X_train_enc.columns.tolist(), os.path.join(model_folder, "train_columns_xgb.pkl"))
print("XGBoost model, scaler, and training columns saved successfully under 'assistant/models'.")


def predict_candidate_xgb(new_candidate_dict):
    """
    Given a dictionary for a new candidate with keys:
      - Occupation, Age_Bracket, Gender, Ethnicity, Education_Level,
      - Age, Experience, Skill_Score, Quality
    This function preprocesses the candidate data using the saved scaler and training columns,
    then uses the saved XGBoost model to predict whether the candidate would be hired.
    ...
    train_columns = joblib.load(os.path.join(model_folder, "train_columns_xgb.pkl"))
    df_new_enc = df_new_enc.reindex(columns=train_columns, fill_value=0)
    scaler_loaded = joblib.load(os.path.join(model_folder, "scaler_xgb.pkl"))
    df_new_scaled = pd.DataFrame(scaler_loaded.transform(df_new_enc), columns=df_new_enc.columns)
    model_loaded = joblib.load(os.path.join(model_folder, "xgboost_model.pkl"))
...
prediction = predict_candidate_xgb(new_candidate)
print("\nNew Candidate Prediction (XGBoost):")
print(prediction)
